...
export default function ProviderIntelligence() {
      ...
      if (cRes.status === 'fulfilled' && Array.isArray(cRes.value)) {
        setAllClaims(cRes.value.map(c => {
          ...
          return {
            ...
            patient: c.patient?.name || c.raw_extracted_features?.patient_name || `Patient #${c.patient_id}`,
            amount: parseFloat(c.total_billed_amount || 0),
            status: c.status === 'paid' ? 'approved' : (c.status === 'denied' ? 'rejected' : c.status),
            riskScore: scoreVal,
            riskLevel: risk?.risk_level?.toLowerCase() || (scoreVal >= 85 ? 'critical' : scoreVal >= 65 ? 'high' : scoreVal >= 35 ? 'medium' : 'low'),
          };
        }));
      }
      setLoading(false);
  ...
  const providersWithLiveStats = providers.map(p => {
    const pClaims = allClaims.filter(c => c.providerId === p.id);
    let riskScore = p.riskScore || 25;
    if (pClaims.length > 0) {
  ...
  const filtered = providersWithLiveStats.filter(p => {
  ...
  const provClaims = selected ? (selected.pClaims || allClaims.filter(c => c.providerId === selected.id)) : [];

...
import { claimsAPI, mlAPI } from '../../services/api';
...
const insights = [
  {
    ...
    description: 'Procedure codes billed separately instead of using standard bundle codes. Flagged by ML IsolationForest Model B.',
  ...
  },
  {
    ...
    description: 'Multiple claims within 30 days for similar procedure types. High probability of duplicate submission.',
    provider: 'Healthcare Provider Facility',
    confidence: 82,
  },
  {
    ...
    description: 'Inpatient claims exceeding regional Medicare peer average. Peer comparison audit initiated.',
  ...
  },
  {
    ...
    description: 'High documentation compliance achieved over recent submissions. No active flags.',
...
export default function AIAnalysis() {
  ...
  const [liveClaimCount, setLiveClaimCount] = useState(0);
  const [estSavings, setEstSavings] = useState('$0');
  const [mlStatus, setMlStatus] = useState('Online');

  const loadMetrics = async () => {
    try {
      const claimsData = await claimsAPI.getAll();
      if (claimsData && claimsData.length) {
        setLiveClaimCount(claimsData.length);
        const total = claimsData.reduce((acc, c) => acc + Number(c.total_billed_amount || 0), 0);
        setEstSavings(`$${Math.round(total * 0.25).toLocaleString()}`);
      }
      const health = await mlAPI.getHealth();
      if (health && health.status) {
        setMlStatus(health.status === 'healthy' ? 'Active' : health.status);
      }
    } catch {
      /* fallback gracefully */
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'Database Claims Analyzed', value: String(liveClaimCount), color: 'text-rose-700', bg: 'bg-rose-50' },
          { label: 'Est. Savings at Risk', value: estSavings, color: 'text-emerald-700', bg: 'bg-emerald-50' },
          { label: 'ML Engine Status', value: mlStatus, color: 'text-violet-700', bg: 'bg-violet-50' },

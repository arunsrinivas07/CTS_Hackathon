...
export default function ExecutiveDashboard() {
  ...
  const invDisplay = investigations.map(i => {
    const pObj = providers.find(p => p.id === i.provider_id);
    const uObj = users.find(u => u.id === i.assigned_to);
    return {
      id: i.id,
      provider: pObj ? (pObj.name || pObj.facility_name) : (i.claim_id ? `Medical Facility #${i.claim_id}` : 'Medical Center'),
      ...
      investigatorName: uObj ? uObj.full_name : (i.assigned_to ? `Agent #${i.assigned_to}` : null),
    };
  });

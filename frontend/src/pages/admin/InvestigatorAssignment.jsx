...
export default function InvestigatorAssignment() {
  ...
  const fetchUsersAndInvs = () => {
    ...
    usersAPI.getAll()
      .then(users => {
        if (Array.isArray(users) && isMounted) {
          const invUsers = users.filter(u => {
            const r = (u.role || '').toLowerCase();
            return r.includes('invest') || u.role_id === 2 || r === 'admin';
          });
          const mapped = invUsers.map(u => ({
            ...
            avatar: (u.full_name || u.email).slice(0, 2).toUpperCase(),
          }));
    ...
    claimsAPI.getAll()
        ...
        if (Array.isArray(res) && isMounted) {
          const mapped = res.map(c => {
            ...
            return {
              ...
              investigatorId: c.assigned_to ? `u${c.assigned_to}` : null,
      ...
      .catch(err => console.warn('Could not load claims in InvestigatorAssignment:', err));
  };
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <div className="w-48">
                        <Select
                          ...
                          onChange={(val) => handleAssign(inv.id, val)}
                          placeholder="Assign to…"
                          options={investigatorList.filter(i => i.status === 'active').map(i => ({
                            value: i.id,
                            label: `${i.name} (${getInvestigatorLoad(i.id)} open)`
                          }))}
                        />
                      </div>
                      {assignments[inv.id] && (

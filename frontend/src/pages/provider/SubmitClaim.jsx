...
const blank = {
  patientName: '', beneId: '', dob: '', gender: '', insurance: '',
...
export default function SubmitClaim() {
  ...
  const handleFileUpload = async (uploadedFiles) => {
    ...
    const docFile = fileList[0];
    setExtracting(true);
    setExtractStatus(`Extracting metadata from ${docFile.name} using OCR parser...`);

    try {
      const res = await claimsAPI.extractFromDocument(docFile);
      if (res && res.extracted_features) {
        const feat = res.extracted_features;
        setForm(prev => ({
          ...prev,
          patientName: feat.patient_name || feat.patient || prev.patientName,
          beneId: feat.bene_id || prev.beneId,
          dob: feat.dob || feat.date_of_birth || prev.dob,
          gender: feat.gender || feat.sex || prev.gender,
          providerNpi: feat.provider_id || prev.providerNpi,
          atPhysnNpi: feat.at_physn_npi || feat.provider_id || prev.atPhysnNpi,
          admissionDate: feat.claim_start_date || prev.admissionDate,
          dischargeDate: feat.claim_end_date || feat.claim_start_date || prev.dischargeDate,
          claimType: feat.claim_type ? (feat.claim_type.charAt(0).toUpperCase() + feat.claim_type.slice(1)) : prev.claimType,
          totalAmount: feat.clm_tot_chrg_amt != null ? String(feat.clm_tot_chrg_amt) : prev.totalAmount,
          paymentAmount: feat.clm_pmt_amt != null ? String(feat.clm_pmt_amt) : prev.paymentAmount,
          lineCount: feat.line_count != null ? String(feat.line_count) : prev.lineCount,
          diagnosisCount: feat.diag_count != null ? String(feat.diag_count) : prev.diagnosisCount,
          procedureCount: feat.proc_count != null ? String(feat.proc_count) : prev.procedureCount,
          state: feat.state || prev.state,
          diagnosis: feat.primary_diagnosis || feat.diagnosis || prev.diagnosis,
          icdCode: feat.icd_code || feat.icd10 || prev.icdCode,
          primaryCpt: feat.primary_cpt || feat.cpt_code || prev.primaryCpt,
        }));
        setExtractStatus(`Successfully auto-filled claim fields from ${docFile.name}!`);
      }
    } catch (err) {
      console.warn('Document extraction warning:', err);
      setExtractStatus(`Attached ${docFile.name}. (Form can be filled manually)`);
    } finally {
      setExtracting(false);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const claimNumber = `CLM-2026-${randNum}`;
      ...
      const rawFeatures = {
        ...
        clm_tot_chrg_amt: parseFloat(form.totalAmount) || 0,
        clm_pmt_amt: form.paymentAmount ? parseFloat(form.paymentAmount) : 0,
        primary_diagnosis: form.diagnosis,
        icd_code: form.icdCode,
        primary_cpt: form.primaryCpt,
        state: form.state,
        line_count: Number(form.lineCount) || 1,
        diag_count: Number(form.diagnosisCount) || 1,
        proc_count: Number(form.procedureCount) || 1,
      };
      ...
      const claimPayload = {
        ...
        claim_type: (form.claimType || 'Outpatient').toLowerCase(),
        ...
        line_count: Number(form.lineCount) || 1,
        state: form.state,
        raw_extracted_features: rawFeatures,
      };
      ...
      setLoading(false);
      setSubmitError(err.message || 'Failed to submit claim. Please check your credentials or form values.');
    }
  };
    ...
    <div className="w-full mx-auto space-y-6" style={{ maxWidth: 'calc(100% - 48px)', width: '1240px' }}>
      {submitError && (
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-sm text-rose-800 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setSubmitError('')} className="text-xs font-bold text-rose-600 underline">
            Dismiss
          </button>
        </div>
      )}
        ...
        <div className="flex flex-col lg:order-2">
          <div className="card-p upload-documents-card flex flex-col">
            ...
            <label className="upload-documents-dropzone flex flex-col items-center justify-center h-36 rounded-xl cursor-pointer transition-all border-2 border-dashed border-slate-200 hover:border-rose-500 hover:bg-rose-50 flex-shrink-0">
              ...
              <span className="text-xs mt-1 text-slate-400">PDF, JPG, PNG, TXT · Supporting evidence requested</span>
              <input type="file" accept=".pdf,.jpg,.jpeg,.png,.txt" multiple className="hidden" onChange={e => handleFileUpload(e.target.files)} />
            </label>

            {extractStatus && (
              <div className="mt-3 p-2.5 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-800 flex items-center gap-2">
                {extracting && <Loader2 size={13} className="animate-spin text-amber-600" />}
        ...
        <form id="submit-claim-form" onSubmit={handleSubmit} className="space-y-6 lg:order-1">
          ...
          <div className="card-p">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <SelectField label="Patient Gender" name="gender" value={form.gender} onChange={e => set('gender', e.target.value)} options={['Male', 'Female', 'Other']} />

...
# ── path bootstrap ────────────────────────────────────────────────────────────
_HERE  = Path(__file__).resolve().parent       # c:\cts_hackathon\backend
_ROOT  = _HERE.parent                          # c:\cts_hackathon
_V2    = _ROOT / "version2_xg" / "backend2"   # c:\cts_hackathon\version2_xg\backend2

for _p in [str(_HERE), str(_ROOT), str(_V2)]:
...
if not _PKL_V2.exists():
    _PKL_V2 = _V2 / "models" / "xgboost_fraud_v2" / "xgboost_fraud_model_v2.pkl"
...
from backend.provider_feature_engine import (
    preprocess_claims,
...
from backend.feature_engine import build_model_b_features, features_to_array

# ── Import LEIE checker ────────────────────────────────────────────────────────
from backend.leie_checker import get_leie_index, lookup as leie_lookup, normalise_npi
...
def _get_peer_lookup() -> pd.DataFrame:
    ...
    if not peer_cache.exists():
        peer_cache = _V2 / "models" / "xgboost_fraud_v2" / "cms_peer_benchmarks.csv"

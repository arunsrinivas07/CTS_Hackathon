...
export default function ClaimTimeline() {
  ...
  const [selected, setSelected] = useState(initialClaims[0]);
      ...
      .then(res => {
        if (Array.isArray(res) && res.length > 0) {
          const mapped = res.map(c => ({
            ...
            diagnosis: c.claim_type ? `${c.claim_type} Care` : 'Clinical Diagnosis',
            icdCode: 'ICD-10',
            ...
            investigatorName: c.status === 'flagged' ? 'Ram Patel' : null,
          ...
          {selected ? (
            <>
              ...
              <div className="bg-[#FFFBEB] border border-[#FDE68A] rounded-xl p-4 mb-5">
                <p className="font-bold text-[#92400e] mb-1.5">
                  Stage 4 of 5: Clinical Review
                </p>
                <p className="text-sm text-slate-600 leading-relaxed">
                  Your uploaded medical records have been received and are currently under verification by the carrier SIU unit.
                </p>
              </div>

              <div className="space-y-2.5 text-sm">
                <p><span className="font-bold text-slate-700">Expected Adjudication:</span> <span className="text-slate-500">2–3 Business Days</span></p>
                <p><span className="font-bold text-slate-700">Assigned Investigator:</span> <span className="text-slate-500">Sarah Mitchell</span></p>
                <p><span className="font-bold text-slate-700">Inquiry Tracking:</span> <span className="text-slate-500">#CG10234</span></p>
              </div>
            </>
          ) : (

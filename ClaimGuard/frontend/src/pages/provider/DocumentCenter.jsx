  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Verified',     count: documents.filter(d => d.status === 'verified').length,     card: 'card-sage' },
          { label: 'Under Review', count: documents.filter(d => d.status === 'under_review').length, card: 'card-amber' },
          { label: 'Flagged',      count: documents.filter(d => d.status === 'flagged').length,      card: 'card-brown' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card-p space-y-4">
          ...
          <div>
            ...
            <input className="input" placeholder="CLM-2024-0081" value={claimId} onChange={e => setClaimId(e.target.value)} />
          </div>
          <div>
            <label className="label">Document Type</label>
            <select className="select" value={docType} onChange={e => setDocType(e.target.value)}>
              <option value="">Select type…</option>
              {['Claim Form', 'Medical Records', 'Operative Report', 'Lab Report', 'Authorization', 'Other'].map(o => (
        ...
        <div className="card overflow-hidden lg:col-span-2">
          <div className="px-5 py-4 border-b border-slate-100">
            <p className="section-title">All Documents ({documents.length})</p>
          </div>

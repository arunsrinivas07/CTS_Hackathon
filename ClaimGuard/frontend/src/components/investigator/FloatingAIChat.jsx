...
import { claims, investigations, documents, providers } from '../../data/mockData';
...
function resolveContext(pathname) {
  ...
  if (pathname.includes('/investigations/') || pathname.includes('/investigator/case/')) {
    ...
    const cl = claims.find(c => c.id === inv.claimId);
    const prov = mockProviders.find(p => p.id === inv.providerId) || mockProviders[0];
    return {
      type: 'CLAIM',
      title: `Claim Assistant · ${inv.claimId}`,
      badge: `CLAIM · ${inv.claimId}`,
      id: inv.claimId,
      entity: { inv, cl, prov }
    };
  }
  ...
  if (pathname.includes('/investigator/documents')) {
    ...
    const doc = claimId ? documents.find(d => d.claimId === claimId) : documents[0];
    return {
      type: 'DOCUMENT',
      title: doc ? `Evidence Assistant · ${doc.claimId}` : 'Evidence Assistant',
      badge: doc ? `DOCUMENT · ${doc.id}` : 'DOCUMENT',
      id: doc?.id,
      entity: doc
    };
  }
  ...
  if (pathname.includes('/investigator/providers')) {
    return {
      type: 'PROVIDER',
      title: 'Provider Assistant · Riverside Medical Center',
      badge: 'PROVIDER · PRV-001',
      id: 'PRV-001',
      entity: providers
    };
  }
...
3. **Pre-Authorization Documentation** (Medium Priority).`;
    ...
    if (context.type === 'PROVIDER') {
      if (qLower.includes('why') || qLower.includes('risk')) {
        return `**Riverside Medical Center** has a risk profile flagged as **High (72/100)**. The profile shows a billing frequency percentile of **99.8th percentile in the region** for Urine Drug Screen CPT codes.`;
      }
      if (qLower.includes('pattern') || qLower.includes('unusual')) {
        return `Unusual billing patterns include a cluster of 14 high-value claims submitted within a tight 60-day window, exceeding standard billing frequencies for similar peer clinics.`;
      }
      if (qLower.includes('compare') || qLower.includes('peer')) {
        return `**Peer Comparison:**
- Billed CPT 80307 average: **$4,000**
- Observed Regional Average: **$380**
- Variance ratio: **11.1x Above regional average**`;
      }
      return `For provider analysis, Riverside Medical Center and Northside Neurological currently present the highest potential recovery exposure in our active queues.`;
    }

    if (context.type === 'DOCUMENT') {
      if (qLower.includes('why') || qLower.includes('flagged') || qLower.includes('operative')) {
        return `The Operative Report (DOC-003) is flagged due to:
1. Missing surgeon signature authentication.
2. Procedure date inconsistencies (service date listed as July 12, intake logged as July 10).`;
      }
      if (qLower.includes('verification') || qLower.includes('ocr') || qLower.includes('result')) {
        return `Text scanning and digital stamps have been validated, but document timeline details still require investigator verification to align intake records with billed service dates.`;
      }
      return `Document Verification is checking digital credentials and procedure codes against operative notes.`;
    }

    if (context.type === 'REPORT') {
      if (qLower.includes('summarize') || qLower.includes('previous') || qLower.includes('audit')) {
        return `Audit Report **REP-2024-001** summary:
- **Provider:** Riverside Medical Center
- **Key Finding:** Pre-payment hold requested on CPT 80307 ($4,000 billed vs $380 peer).
- **Status:** Marked Suspicious by investigator Ram Patel.`;
      }
    ...
    if (qLower.includes('critical') || qLower.includes('priority')) {
      return `There are currently **2 Critical priority cases** in the active queue:
1. **INV-2024-0039** (Upcoding, Riverside, $91,750)
2. **CLM-2024-0075** (Cardiac Bypass, $91,750)
Both warrant immediate documentation review.`;
    }
    ...
    return `I've analyzed your query: **"${q}"** in Global Mode. I can locate high-risk providers, summarize critical cases, and explain risk drivers. Try asking about Riverside Medical Center or critical investigations.`;
  };
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-2.5 items-start ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${
                  msg.role === 'bot' ? 'bg-rose-100 text-rose-700' : 'bg-slate-200 text-slate-700'
                }`}>
                  {msg.role === 'bot' ? 'AI' : 'ME'}
                </div>

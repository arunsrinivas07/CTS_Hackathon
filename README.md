...
**CareGuard AI** is an enterprise-grade Medicare Fraud, Waste, and Abuse (FWA) detection system. It combines **unsupervised point-of-service anomaly detection across all 7 CMS medical claim settings**, **dedicated Part D prescription drug event (PDE) anomaly detection**, **longitudinal beneficiary patient trajectory modeling**, and **deterministic HHS OIG regulatory compliance screening**.
...
```
  ...
  │ (Carrier, Inpatient,    │   │ • Ingests Claims + Beneficiary Longitudinal Trajectory  │              │ (Claim-Level Medical)     │
  │ Outpatient, DME, etc.)  │   │ • Median Imputer → StandardScaler → IsolationForest     │              │ • 59 Features             │
  └─────────────────────────┘   │ • 59 Features: Financials, Peer Z-Scores, 30D/90D Spends│              └─────────────┬─────────────┘
                                └─────────────────────────────────────────────────────────┘                            │
                                                                                                                       │
  ┌─────────────────────────┐   ┌─────────────────────────────────────────────────────────┐                            │
  │ Part D Prescription     │──►│ MODEL C PIPELINE (scripts/train_model_c_pde.py)         │ ───────────► │ POST /api/v1/predict      │
  │ Drug Events (PDE)       │   │ • Ingests PDE + Patient Prescription History (30D/90D)  │              │ (Part D Rx Anomaly)       │
  └─────────────────────────┘   │ • Median Imputer → StandardScaler → IsolationForest     │              │ • 32 Features             │
...
2. **Beneficiary Longitudinal Trajectory Engine (`scripts/build_bene_history_features.py`)**
   - Ingests Beneficiary Master records (`beneficiary_YYYY.csv`) to resolve patient demographics (`BENE_AGE_AT_CLAIM`, `SEX_IDENT_CD`, `BENE_RACE_CD`, `STATE_CODE`, `BENE_IS_DECEASED`).
   - Dynamically compiles point-in-time longitudinal patient history (lifetime claim counts, cumulative prior payments, average/max historical reimbursement, days since previous visit) and rolling **30-day and 90-day velocity windows**.
3. **Model B: Point-of-Service Medical Claim Anomaly Detector (`models/model_b_claim/`)**
...
4. **Model C: Part D Prescription Drug Event (PDE) Anomaly Detector (`models/model_c_pde/`)**
...
│   │   ├── beneficiary/                 # Beneficiary Annual Files (2015–2025: Demographics & Enrollment)
...
### Phase 1: Medical Claims Normalization & Beneficiary Feature Pipeline
```bash
# 1. Normalize 7 raw claim CSVs into unified schema (1,888,860 rows)
python scripts/normalize_medical_claims.py

# 2. Build point-in-time longitudinal beneficiary history (Lifetime & 30D/90D windows from Beneficiary files)
...
```bash
# 8. Extract 32 PDE features (unit cost, refill frequency, prescriber volume, patient Rx history)

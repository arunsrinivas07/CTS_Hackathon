...
from agent.counter_analysis import run_counter_analysis
from agent.evidence_evaluator import evaluate_sufficiency
from agent.question_generator import generate_next_question
from agent.report_generator import build_final_report, draft_conclusion
from agent.critic import run_critic
from agent.state import initialize_investigation, new_id
from agent.tool_executor import deterministic_observation_text, execute_tool, record_observation
from agent.tool_router import resolve_tool
from llm.client import LLMClient, LLMError
from schemas.investigation import InvestigationObjective, InvestigationState, InvestigationStatus
from schemas.tool import ToolResultStatus

logger = logging.getLogger("agent.orchestrator")

OBJECTIVES_SYSTEM_PROMPT = """You are the planning module of a healthcare
claims fraud investigation agent. Given a prioritized claim's risk profile,
derive 2-5 concrete investigation objectives specific to THIS claim's actual
risk factors and detected patterns -- do not use generic boilerplate
objectives unrelated to the signals present.

Respond with ONLY a JSON object, no prose, no markdown fences:
{
  "objectives": [
    {"description": "...", "related_risk_factors": ["feature_name", "..."]}
  ]
}
"""
...
def _derive_objectives(state: InvestigationState, llm: LLMClient) -> list[InvestigationObjective]:
    ...
    try:
        raw = llm.complete_json(OBJECTIVES_SYSTEM_PROMPT, "Claim risk profile:\n" + json.dumps(payload, default=str))
    except LLMError as exc:
        logger.error("Objective derivation failed: %s", exc)
        # Safe minimal fallback so the loop can still proceed.
        return [
            InvestigationObjective(
                objective_id=new_id("obj"),
                description="Determine whether the primary ML risk factors reflect a genuine anomaly.",
                related_risk_factors=[rf.name for rf in state.risk_factors],
            )
        ]
    ...
    for o in raw.get("objectives", []):
        objectives.append(
            InvestigationObjective(
                ...
                related_risk_factors=o.get("related_risk_factors", []) or [],
            )
        )

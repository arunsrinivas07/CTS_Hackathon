...
from app.copilot.adapters.database_adapter import ClaimDBTool, ProviderDBTool
from app.copilot.adapters.ml_adapter import MLTool
from app.copilot.adapters.rag_adapter import RAGTool
from app.copilot.adapters.scenario_adapter import ScenarioTool
from app.copilot.schemas.copilot import CitationItemOut, EvidenceItemOut, QuestionType, ToolUsageRecord
from app.schemas.agentic.investigation import Citation, Evidence, InvestigationState, SHAPFactor
...
class ToolRouter:
    ...
    def route(self, question_type: str, question: str, state: InvestigationState) -> Optional[ToolResult]:
        """Route based on string intent name."""
        # Map string intents to internal QuestionType if needed
        intent_map = {
            "EVIDENCE_QUERY": "POLICY_QUESTION",
            "EVIDENCE_GAP": "POLICY_QUESTION",
            "RISK_ASSESSMENT": "ML_EXPLANATION",
            "EXPLANATION": "ML_EXPLANATION",
            "CLAIM_DETAILS": "PROVIDER_HISTORY",
        }
        
        mapped_type = intent_map.get(question_type, question_type)
        handler = self._HANDLERS.get(mapped_type)
        if handler is None:
            return None
    ...
    def _handle_provider_history(self, _question: str, state: InvestigationState) -> ToolResult:
        # Extract provider_id from claim_data
        provider_id = state.claim_data.get("provider_id") or state.claim_data.get("provider_name", "")
        if not provider_id:
            return ToolResult(
                answer="I don't have provider information for this claim.",
                tool_usage=ToolUsageRecord(tool="provider_db_tool", purpose="Retrieve provider historical behavior"),
                confidence=0.3,
            )
            
        history = self._provider_db_tool.get_mri_history(provider_id)
        if not history:
            return ToolResult(
                answer=f"I don't currently have historical provider data available for provider {provider_id}.",
                tool_usage=ToolUsageRecord(tool="provider_db_tool", purpose="Retrieve provider historical behavior"),
                confidence=0.3,
            )
        trend = ", ".join(f"{h.get('period', h.get('month', 'Unknown'))}={h.get('mri_count', h.get('mri_claims', 0))}" for h in history[:6])
        answer = (
            f"Provider {provider_id}'s historical claims: {trend}. "
        )
        new_evidence = Evidence(
            id=f"EV-TOOL-{state.investigation_id}-provider-history",
            summary=f"Provider {provider_id}'s historical claim trend was retrieved.",
            detail=trend,
    ...
    _HANDLERS = {
        ...
        "PROVIDER_HISTORY": _handle_provider_history,
        "ML_EXPLANATION": _handle_ml_explanation,
        "SCENARIO_QUESTION": _handle_scenario,
    }

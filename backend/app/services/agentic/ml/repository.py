...
class LiveMLRepository(BaseMLRepository):
    ...
    def __init__(self, service_url: Optional[str] = None, timeout: float = 10.0) -> None:
        ...
        self.service_url = service_url or getattr(settings, "ml_service_url", "https://careguard-ml-latest.onrender.com")
        self.timeout = timeout

    def get_risk_prediction(self, claim_id: str) -> Optional[MLRiskResponse]:
        import httpx
        from app.llm.config import settings
        from app.services.agentic.database.repository import MockDatabaseRepository

        db = MockDatabaseRepository()
        claim = db._claims.get(claim_id)
        if not claim:
        ...
        try:
            ...
            return MLRiskResponse(
                ...
                risk_level=res_data.get("final_risk_level", res_data.get("ml_risk_level", "HIGH")),
                shap_features=[
                    ShapFeature(
                        feature="provider_id",
                        impact=res_data.get("ml_risk_score", 0.0),
                        direction="increases_risk",
                        description=res_data.get("explanation", "CareGuard ML prediction explanation")
                    )
                ],
                model_version=res_data.get("model_used", "hybrid_model_v1")
            )
    ...
    def simulate_scenario(
        ...
        from app.services.agentic.ml.repository import MockMLRepository

...
from app.schemas.agentic.question import InvestigationQuestion, Observation, ToolCallRecord
from app.schemas.agentic.tool import ToolResultStatus
from app.tools.base import get_tool, is_registered
...
_EVIDENCE_TYPE_MAP = {
    ...
    "ml_verification": EvidenceType.ML_SCORE,
...
def execute_tool(tool_name: str, question: InvestigationQuestion | None, state: InvestigationState) -> ToolCallRecord:
    ...
    from app.agent.tool_router import build_tool_input  # local import avoids circularity
...
def _lift_evidence(tool_name, output, question: InvestigationQuestion | None, state: InvestigationState) -> None:
    ...
    for raw in output.evidence:
        # raw is an EvidenceItem (Pydantic model) — `text` is the primary content field
        description = (
            getattr(raw, "description", None)
            or getattr(raw, "text", None)
            or getattr(raw, "summary", None)
            or ""
        )
        if not description:
            continue  # skip empty evidence items
        ...
        evidence = Evidence(
            ...
            supporting_data=getattr(output, "data", {}) or {},
            confidence=getattr(raw, "confidence", None)
                or getattr(raw, "evidence_score", None)
                or getattr(output, "confidence", 0.5)
                or 0.5,
    ...
    logger.info("[TOOL] Tool: %s | Evidence items lifted: %d", tool_name, lifted_count)
    print(f"[TOOL]\nTool: {tool_name}\nEvidence items: {lifted_count}")

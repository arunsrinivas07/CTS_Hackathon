...
_log = logging.getLogger("feature_engine")
...
def _load_hist_b() -> pd.DataFrame:
    ...
    if _hist_b is None:
        ...
        if not HIST_B_CSV.exists():
            _log.warning("Medical beneficiary history CSV not found: %s. Using empty history.", HIST_B_CSV)
            _hist_b = pd.DataFrame(columns=cols)
            return _hist_b
...
def _load_hist_c() -> pd.DataFrame:
    ...
    if _hist_c is None:
        ...
        if not HIST_C_CSV.exists():
            _log.warning("PDE features CSV not found: %s. Using empty history.", HIST_C_CSV)
            _hist_c = pd.DataFrame(columns=cols + ["SRVC_DATE"])
            return _hist_c

...
def _evidence_question(state: InvestigationState, _q: str) -> ResolvedAnswer:
    ...
    lines = [f"{e.summary} ({e.source})" for e in state.evidence]
...
def _counter_evidence(state: InvestigationState, _q: str) -> ResolvedAnswer:
    ...
    lines = [f"{ce.summary} ({ce.source})" for ce in state.counter_evidence]
...
def _provider_history(state: InvestigationState, _q: str) -> ResolvedAnswer:
    ...
    history_evidence = [e for e in state.evidence if "history" in e.summary.lower() or "12 month" in e.summary.lower()]
    if history_evidence:
        lines = [f"{e.summary} ({e.source})" for e in history_evidence]
...
def _ev_out(e) -> EvidenceItemOut:
    return EvidenceItemOut(id=e.id, summary=e.summary, source=e.source)


def _cit_out(c) -> CitationItemOut:
    return CitationItemOut(id=c.id, title=c.title, source=c.source, excerpt=c.excerpt, url=c.url)
